...
foreach($dh as $file) {
  ...
  if(is_file('./showpad/'.$file)) {
    ...
    if(($episode[$i]['file']['hash'] != $returnarray[0]['filehash'])&&($episode[$i]['file']['size'] > 400)) {
      $now = time();
      ...
      $insertEpisode['subject'] = $episode[$i]['subject'];
      $insertEpisode['filetime'] = $now;
      $insertEpisode['filesize'] = $episode[$i]['file']['size'];
